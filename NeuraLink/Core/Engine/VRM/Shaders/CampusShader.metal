...
struct CampusShadowVertexIn {
    float3 position [[attribute(0)]];
};
struct CampusVertexIn {
    float3 position [[attribute(0)]];
...
vertex float4 campus_shadow_vertex(
    CampusShadowVertexIn       in [[stage_in]],
    constant CampusShadowUniforms &u [[buffer(1)]],
...
vertex CampusVertexOut campus_vertex(
    CampusVertexIn             in [[stage_in]],
    constant CampusUniforms    &u [[buffer(1)]],
    constant float4x4 &modelMatrix [[buffer(2)]]
) {
...
static float campusSampleShadow(
...
fragment float4 campus_fragment(
    CampusVertexOut            in              [[stage_in]],
    constant CampusUniforms    &u               [[buffer(1)]],
    ...
    texture2d<float>           shadowMap       [[texture(1)]],  // wide campus shadow
    ...
    float campusShadow = (u.campusParams.x > 0.0f)
        ? campusSampleShadow(shadowMap,    shadowSmp, in.worldPos,
                           u.lightViewProjection,    u.campusParams.x)
        : 0.0f;
    // VRM tight shadow map: character casts a shadow on the campus ground.
    float vrmShadow = campusSampleShadow(vrmShadowMap, shadowSmp, in.worldPos,
                                       u.vrmLightViewProjection, 1.5f);
    float shadow = max(campusShadow, vrmShadow);
    ...
    float3 artificial = float3(0.10f, 0.06f, 0.02f) * (1.0f - dayFactor) * (1.0f - hemi);
    ambient += artificial;
