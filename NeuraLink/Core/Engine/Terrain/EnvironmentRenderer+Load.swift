...
extension EnvironmentRenderer {
    ...
    func load(url: URL) async throws {
            ...
            for primitive in mesh.primitives {
                ...
                if let mat = mat {
                    ...
                    if let texIdx = baseColorTexIdx {
                        texture = await Self.cachedTexture(
                            texIdx, sRGB: true, cache: &textureCache, loader: texLoader)
                    }
        ...
        // `rotY` is a pure yaw about the vertical axis through the origin (the
        // viewer), so it turns the scene around the avatar without touching Y.
    ...
    private static func cachedTexture(
        _ index: Int, sRGB: Bool, cache: inout [String: MTLTexture], loader: TextureLoader
    ) async -> MTLTexture? {
        // Key by index AND colour space: one image may be referenced both as an
        // sRGB colour map and a linear data map (normal / MR); they must not alias.
        let key = "\(index):\(sRGB)"
