...
size_t llama_bridge_save_kv_state(LlamaBridgeHandle *handle, const char *path) {
  if (!handle || !handle->ctx || !path) {
    return 0;
  }
  if (handle->kv_tokens.empty()) {
    return 0;
  }
  return llama_state_seq_save_file(handle->ctx, path,
                                   /*seq_id=*/0, handle->kv_tokens.data(),
                                   handle->kv_tokens.size());
}

int32_t llama_bridge_load_kv_state(LlamaBridgeHandle *handle,
                                   const char *path) {
  if (!handle || !handle->ctx || !path) {
    return 0;
  }
...
void llama_bridge_get_pld_stats(LlamaBridgeHandle *handle, int32_t *out_rounds,
                                int32_t *out_hits) {
  if (!handle) {
    return;
  }
  if (out_rounds) {
    *out_rounds = handle->last_pld_rounds;
  }
  if (out_hits) {
    *out_hits = handle->last_pld_hits;
  }
}
