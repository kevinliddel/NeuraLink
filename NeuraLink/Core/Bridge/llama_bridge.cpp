...
int32_t llama_bridge_apply_chat_template(LlamaBridgeHandle* handle, const char* const* roles,
    ...
    if (!handle || !handle->model || !roles || !contents || n_messages <= 0 || !out_buf || out_buf_size <= 0) {
        return -1;
    }
...
static int token_to_str(llama_model* model, llama_token tok, char* buf, int buf_size) {
...
static bool sync_kv_for_prompt(LlamaBridgeHandle* h, const std::vector<llama_token>& new_tokens) {
    ...
    h->last_prefill_ms     = std::chrono::duration<double, std::milli>(prefill_end - prefill_start).count();
...
static size_t find_pld_match(const std::vector<llama_token>& tokens, int32_t n) {
    ...
    for (size_t i = last_start; i > 0; --i) {
        ...
        if (std::memcmp(tokens.data() + start, needle, un * sizeof(llama_token)) == 0) { return start + un; }
    }
    return kNoMatch;
}
...
static bool emit_utf8_safe(std::string& buf, const char* piece, int piece_len, LlamaTokenCallback on_token,
                           void* user_ctx) {
...
static void flush_utf8_remaining(std::string& buf, LlamaTokenCallback on_token, void* user_ctx) {
...
static void generate_standard(LlamaBridgeHandle* h, std::vector<llama_token>& new_tokens, int32_t max_new_tokens,
...
static bool pld_emit(LlamaBridgeHandle* h, llama_token tok, LlamaTokenCallback on_token, void* user_ctx,
                     char* piece_buf, int buf_size, std::string& utf8_buf) {
    const auto* vocab = llama_model_get_vocab(h->model);
    if (llama_vocab_is_eog(vocab, tok)) { return false; }
    int piece_len = token_to_str(h->model, tok, piece_buf, buf_size);
    if (piece_len > 0) {
        if (!emit_utf8_safe(utf8_buf, piece_buf, piece_len, on_token, user_ctx)) { return false; }
    }
    return true;
}

static void generate_pld(LlamaBridgeHandle* h, std::vector<llama_token>& new_tokens, int32_t max_new_tokens,
                         LlamaTokenCallback on_token, LlamaFinishCallback on_finish, void* user_ctx) {
    char piece_buf[512] = {};
    std::string utf8_buf;
    int generated = 0;
    ...
    while (generated < max_new_tokens && !h->cancel_flag.load()) {
        ...
        const int32_t budget   = std::min(h->pld_n_draft, max_new_tokens - generated);
        const size_t available = h->kv_tokens.size() > match_pos ? h->kv_tokens.size() - match_pos : 0;
        const int32_t n_draft  = std::min(budget, static_cast<int32_t>(available));
        ...
        std::vector<llama_token> drafts(h->kv_tokens.begin() + match_pos, h->kv_tokens.begin() + match_pos + n_draft);
        ...
        for (int i = 1; i < n_draft && !stop_round; ++i) {
            ...
            if (t == drafts[i]) {
                ...
                if (!pld_emit(h, t, on_token, user_ctx, piece_buf, sizeof(piece_buf), utf8_buf)) { stop_round = true; }
                break;
            }
        }
