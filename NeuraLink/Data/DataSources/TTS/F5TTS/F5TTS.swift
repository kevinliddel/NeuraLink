...
public final class F5TTS: @unchecked Sendable {
    ...
    public static func load(modelPath: String, vocabPath: String, vocoderPath: String) throws -> F5TTS {
        ...
        nlLog("[F5-TTS/load] Step 1: streaming fp16 load from \(URL(fileURLWithPath: modelPath).lastPathComponent)…", level: .info)
        let f16Transformer: [String: MLXArray]
        do {
            f16Transformer = try loadFloat16Weights(url: URL(fileURLWithPath: modelPath))
            nlLog("[F5-TTS/load] Step 1a: loaded \(f16Transformer.count) tensors (fp16)", level: .info)
        } catch {
            nlLog("[F5-TTS/load] Step 1a FAILED: \(error)", level: .error)
            throw error
        }
        ...
        do {
            ...
            nlLog("[F5-TTS/load] Step 1b: DiT parameters updated", level: .info)
        } catch {
            let sampleKeys = Array(f16Transformer.keys.sorted().prefix(10))
            nlLog("[F5-TTS/load] Step 1b FAILED: \(error)", level: .error)
            nlLog("[F5-TTS/load]   First 10 weight keys: \(sampleKeys)", level: .error)
            throw error
        }

        // ── Step 2: Load Vocos weights as fp16 ───────────────────────────────
        nlLog("[F5-TTS/load] Step 2: streaming fp16 load from \(URL(fileURLWithPath: vocoderPath).lastPathComponent)…", level: .info)
        let f16Vocoder: [String: MLXArray]
        do {
            f16Vocoder = try loadFloat16Weights(url: URL(fileURLWithPath: vocoderPath))
            nlLog("[F5-TTS/load] Step 2a: loaded \(f16Vocoder.count) vocoder tensors (fp16)", level: .info)
        } catch {
            nlLog("[F5-TTS/load] Step 2a FAILED: \(error)", level: .error)
            throw error
        }
        ...
        do {
            ...
            nlLog("[F5-TTS/load] Step 2b: Vocos parameters updated (\(f16Vocoder.count) tensors)", level: .info)
        } catch {
            let sampleKeys = Array(f16Vocoder.keys.sorted().prefix(10))
            nlLog("[F5-TTS/load] Step 2b FAILED: \(error)", level: .error)
            nlLog("[F5-TTS/load]   First 10 vocoder keys: \(sampleKeys)", level: .error)
            throw error
        }

        // ── Step 3: Build tokenizer ───────────────────────────────────────────
        nlLog("[F5-TTS/load] Step 3: reading vocab from \(URL(fileURLWithPath: vocabPath).lastPathComponent)…", level: .info)
        let vocabText: String
        do {
            vocabText = try String(contentsOfFile: vocabPath, encoding: .utf8)
        } catch {
            nlLog("[F5-TTS/load] Step 3 FAILED (read vocab): \(error)", level: .error)
            throw error
        }
        ...
        nlLog("[F5-TTS/load] Step 3: vocab size = \(vocab.count) tokens", level: .info)
        let tokenizer = Tokenizer(vocab: vocab)

        nlLog("[F5-TTS/load] ── load complete", level: .info)
    ...
    public func generate(
        ...
        nlLog("[InternalF5] generate() — \(text.count) chars, \(steps) steps", level: .info)
        ...
        nlLog("[InternalF5] Reference: \(refSamples.count) samples (\(String(format: "%.1f", refDuration))s, capped)", level: .info)
        ...
        nlLog("[InternalF5] Reference mel: \(refFrames) frames", level: .info)
        ...
        nlLog("[InternalF5] Target: \(targetFrames) frames (~\(String(format: "%.1f", Double(targetFrames) / 93.75))s)", level: .info)

        // 4. Flow-matching ODE: integrate from t=0 (noise) → t=1 (data).
        //    Reference portion stays fixed; only the target portion diffuses.
        //    The solver MLX.eval()s every step so the lazy graph doesn't
        //    accumulate ~steps × model_size and exhaust RAM.
        let initialNoise = MLXRandom.normal([1, targetFrames, 80]).asType(.float16)
        nlLog("[InternalF5] Starting ODE (\(steps) steps, total seq = \(refFrames + targetFrames) frames)…", level: .info)

        let solver = ODESolverFactory.make(.euler)
        let targetXt = solver.integrate(from: initialNoise, steps: steps) { x, t in
            // Full-sequence input: [1, refFrames + targetFrames, 80]
            let xt = MLX.concatenated([refMelBatch, x], axis: 1)
            let velocity = transformer(x: xt, t: MLXArray([t]))
            // velocity[0] → [totalFrames, 80]; [refFrames...] → [targetFrames, 80]
            return velocity[0][refFrames...].expandedDimensions(axis: 0)
        }
        nlLog("[InternalF5] ODE done — decoding through Vocos", level: .info)
        ...
        nlLog("[InternalF5] Output: \(samples.count) samples (\(String(format: "%.2f", Double(samples.count) / 24000.0))s)", level: .info)
        return samples
    }
    ...
    private static func loadFloat16Weights(url: URL) throws -> [String: MLXArray] {
        ...
        nlLog("[F5-TTS/loader] \(out.count) tensors loaded as fp16 from \(url.lastPathComponent)", level: .info)
        return out
    }
}
