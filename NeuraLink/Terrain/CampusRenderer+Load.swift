...
extension CampusRenderer {
    ...
    func load(url: URL) async throws {
        ...
        vrmLog("[CampusRenderer] Scene graph: \(meshInstances.count) mesh instance(s) across all nodes")
        ...
        for (meshIndex, worldTransform) in meshInstances {
            ...
            for primitive in mesh.primitives {
                ...
                var vertices = [CampusVertex](
                    repeating: CampusVertex(position: .zero, normal: [0, 1, 0],
        ...
        vrmLog("[CampusRenderer] Loaded \(groups.count) mesh group(s) (\(textureCache.count) unique texture(s))")
    }
    ...
    private func setupPipelines(config: RendererConfig) {
        ...
        else { vrmLog("[CampusRenderer] Shader functions not found"); return }
