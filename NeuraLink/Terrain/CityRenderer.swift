...
//
//  Loads and renders a single static GLB city environment (city.glb).
...
final class CityRenderer: @unchecked Sendable {

    typealias InstanceConfig = (x: Float, y: Float, z: Float, rotY: Float, scale: Float)

    private let device: MTLDevice
    private var meshGroups: [CityMeshGroup] = []
    ...
    private var blendDepthState: MTLDepthStencilState?
    private var uniformsBuffer: MTLBuffer?
    private var shadowUniformsBuffer: MTLBuffer?

    private var isReady = false
    private let instanceConfig: InstanceConfig

    /// True once the GLB has finished loading. Used by VRMRenderer to suppress
    /// terrain rendering while the city environment is active.
    ...
    func load(url: URL) async throws {
        ...
        for ni in document.scenes?[safe: sceneIndex]?.nodes ?? [] {
            collectMeshInstances(nodeIndex: ni, parentTransform: instanceTransform,
        ...
        for (meshIndex, worldTransform) in meshInstances {
            ...
            for primitive in mesh.primitives {
                ...
                let texIdxStr   = mat.flatMap { $0.pbrMetallicRoughness?.baseColorTexture?.index }
                                     .map { "tex[\($0)]" } ?? "none"
